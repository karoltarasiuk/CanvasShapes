    ...
    describe('CanvasShapes.GeometryTools', function () {
        ...
        it('factorial method', function () {
            ...
            for (i = 0; i < cache.length; i++) {
                expect(CanvasShapes.GeometryTools.factorial(i)).toBe(cache[i]);
            }
        });

        it('binomialCoefficient method', function () {

            var i,
                error = new CanvasShapes.Error(1069),
                data = [
                    [0, 1, 1],
                    [4, 8, 70],
                    [4, 5, 5],
            ...
            expect(function () {
                CanvasShapes.GeometryTools.binomialCoefficient(-1, 1);
            }).toThrow(error);

            expect(function () {
                CanvasShapes.GeometryTools.binomialCoefficient(1, 0);
            }).toThrow(error);

            expect(function () {
                CanvasShapes.GeometryTools.binomialCoefficient(0, 0);
            }).not.toThrow();

            for (i = 0; i < data.length; i++) {
                expect(CanvasShapes.GeometryTools.binomialCoefficient(
                    data[0][0],
                    data[0][1]
                )).toBe(data[0][2]);
            }
        ...
        it('bernstein method', function () {

            var i,
                error = new CanvasShapes.Error(1060),
                data = [
                    [0, 0, 0.5, 1],
                    [0, 0, 0.1, 1],
                    [0, 1, 0.2, 0.8],
            ...
            expect(function () {
                CanvasShapes.GeometryTools.bernstein(0, 0, -0.0001);
            }).toThrow(error);

            expect(function () {
                CanvasShapes.GeometryTools.bernstein(0, 0, 1.0001);
            }).toThrow(error);

            expect(function () {
                CanvasShapes.GeometryTools.bernstein(0, 0, 0.1);
            }).not.toThrow();

            for (i = 0; i < data.length; i++) {
                expect(CanvasShapes.GeometryTools.bernstein(
                    data[0][0],
                    data[0][1],
                    data[0][2]
                )).toBe(data[0][3]);
