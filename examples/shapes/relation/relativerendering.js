...
require([
    ...
    var Showcase = (function () {
        ...
        Showcase = function (width, height, domId) {
            ...
            this.toggleRelativeRendering();
        };

        Showcase.prototype.toggleRelativeRendering = function () {

            var $toggle = $('a.toggle'),
                that = this;

            $toggle.click(function () {
                that.elements.relation.setRelativeRendering(
                    !that.elements.relation.getRelativeRendering()
                );
                that.elements.relation.getSceneInterfaceHandlers()
                    .requestRendering(that.elements.relation);
        ...
        Showcase.prototype.generator = (function () {
            ...
            return function (x) {
                ...
                if (!this.elements.relation.getRelativeRendering()) {
                    width = this.width;
                    height = this.height;
                }

                // sine
                func4 = height / 2 * Math.sin(x / (width / (2 * Math.PI))) + height / 2;
